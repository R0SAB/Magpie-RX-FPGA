...
#(
	...
	parameter ROM_FILE_0 = "file.txt",
	parameter ROM_FILE_1 = "file.txt",
	parameter ROM_FILE_2 = "file.txt",
...
reg signed [TAPS_MSB:0]rom_0[0:ORDER];
reg signed [TAPS_MSB:0]rom_1[0:ORDER];
reg signed [TAPS_MSB:0]rom_2[0:ORDER];
...
reg signed [TAPS_MSB:0]rom_read_0 = 0;
reg signed [TAPS_MSB:0]rom_read_1 = 0;
reg signed [TAPS_MSB:0]rom_read_2 = 0;
...
begin
	...
	if(samp_clk_eg == 2'b00 && samp_clk) rom_addr <= 0;				// ROM address counter
	else
	...
	rom_read_0 <= rom_0[rom_addr];								// Read operation of ROM
	rom_read_1 <= rom_1[rom_addr];								
	rom_read_2 <= rom_2[rom_addr];

	case(bw_in)
		2'd0: rom_read <= rom_read_0;
		2'd1: rom_read <= rom_read_1;
		2'd2: rom_read <= rom_read_2;
	endcase								
...
initial $readmemb(ROM_FILE_0, rom_0);
initial $readmemb(ROM_FILE_1, rom_1);
initial $readmemb(ROM_FILE_2, rom_2);
